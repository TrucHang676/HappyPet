...
// //   const PAGE_SIZE = 10; // Tăng nhẹ số lượng hiển thị vì đã bỏ cột lọc
...
// //         setBranches(arr);
// //       } catch (e) {
// //         console.error('Lỗi load chi nhánh:', e);
// //       }
// //     };
// //     loadBranches();
// //   }, []);
...
// //   // --- HÀM THÊM GIỎ HÀNG (GIỮ NGUYÊN) ---
// //   const handleAddToCart = async (product) => {
// //     const token = localStorage.getItem('token');
// //     if (!token) {
// //       Swal.fire({
// //         icon: 'warning', title: 'Chưa đăng nhập', text: 'Bạn vui lòng đăng nhập để mua hàng nhé!',
// //         showCancelButton: true, confirmButtonText: 'Đăng nhập', cancelButtonText: 'Để sau'
// //       }).then((result) => { if (result.isConfirmed) window.location.href = '/login'; });
// //       return;
// //     }

// //     if (product.TinhTrang === 'Hết hàng') { Swal.fire('Thông báo', 'Sản phẩm này hiện đã hết hàng!', 'warning'); return; }
...
// //     if (!city || !branch) { Swal.fire('Thông báo', 'Vui lòng chọn thành phố và chi nhánh trước khi thêm vào giỏ.', 'warning'); return; }

// //     const tonKho = product.SoLuongTon ?? null;
// //     if (branch && tonKho !== null && Number(tonKho) <= 0) { Swal.fire('Thông báo', 'Chi nhánh này hiện đã hết hàng sản phẩm này!', 'warning'); return; }

// //     try {
// //         let cleanCode = localStorage.getItem('currentOrderCode');
// //         if (cleanCode) cleanCode = String(cleanCode).replace(/\+/g, '').trim();
        
// //         const payload = {
// //             maMH: product.MaMatHang, soLuong: 1, maCN: branch, diaChi: `Nội thành ${city}`, maPhieuHienTai: cleanCode
// //         };
...
// //         }
// //         Swal.fire({ icon: 'success', title: 'Thành công!', text: `${product.TenMatHang} đã thêm vào giỏ.`, showConfirmButton: false, timer: 1500 });
// //     } catch (err) {
...
// //               {[1, 2, 3, 4, 5].map((star) => (
// //                   <span key={star} className="star" style={{ color: star <= Math.round(rating) ? '#ffc107' : '#e4e5e9', fontSize: '18px' }}>★</span>
// //               ))}
// //               <span className="rating-count" style={{fontSize: '12px', color: '#666', marginLeft: '5px'}}>({count} đánh giá)</span>
// //           </div>
// //       );
// //   };
...
// //   const cityOptions = useMemo(() => Array.from(new Set(branches.map(b => detectCityFromAddress(getBranchAddress(b))).filter(Boolean))), [branches]);
// //   const branchesInCity = useMemo(() => branches.filter(b => detectCityFromAddress(getBranchAddress(b)) === selectedCity), [branches, selectedCity]);
// //   const selectedBranchObj = selectedBranch ? branches.find(b => getBranchId(b) === selectedBranch) : null;

// //   // Logic sync saved branch
// //   useEffect(() => {
// //     let mounted = true;
// //     const resolveSavedBranch = async () => {
// //       if (!selectedBranch) { if (mounted) setSavedBranchLabel(null); return; }
// //       const found = branches.find(b => String(getBranchId(b)).trim() === String(selectedBranch).trim());
// //       if (found) { if (mounted) setSavedBranchLabel(`${getBranchId(found)} - ${getBranchAddress(found)}`); return; }
// //       if (attemptedFetchRef.current) { if (mounted) setSavedBranchLabel(selectedBranch); return; }
...
// //         if (f) { if (mounted) setSavedBranchLabel(`${getBranchId(f)} - ${getBranchAddress(f)}`); return; }
// //         if (mounted) setSavedBranchLabel(selectedBranch);
// //       } catch (e) { if (mounted) setSavedBranchLabel(selectedBranch); }
// //     };
// //     resolveSavedBranch();
// //     return () => { mounted = false; };
// //   }, [selectedBranch, branches]);

// //   const handleChooseCity = (val) => { const v = (val || '').trim(); setSelectedCity(v); setSelectedBranch(''); localStorage.setItem('shipCity', v); localStorage.removeItem('shipBranch'); };
// //   const handleChooseBranch = (val) => { const v = (val || '').trim(); setSelectedBranch(v); localStorage.setItem('shipBranch', v); };
// //   const handleClearBranch = () => { setSelectedCity(''); setSelectedBranch(''); localStorage.removeItem('shipCity'); localStorage.removeItem('shipBranch'); };
...
// //   const goToPage = (p) => { if (p < 1 || p > totalPages) return; setPage(p); window.scrollTo({ top: 0, behavior: 'smooth' }); };
...
// //         {CATEGORIES.map(cat => (
// //           <div key={cat.key} className={`cat-item ${selectedCategory === cat.key ? 'active' : ''}`} onClick={() => setSelectedCategory(cat.key)} style={{ cursor: 'pointer' }} title={cat.label}>
...
// //         {/* CONTENT (Bây giờ nó sẽ full màn hình) */}
// //         <div className="shop-content" style={{width: '100%', maxWidth: '1200px', margin: '0 auto'}}>
...
// //           {/* CHỌN CHI NHÁNH */}
// //           <div style={{ background: '#fff3cd', border: '1px solid #ffeeba', padding: '12px 14px', borderRadius: '10px', margin: '12px 0 16px 0' }}>
// //             <b>⚠️ Trung tâm chỉ cho phép giao nội thành.</b>
// //             <div style={{ marginTop: 6 }}>Vui lòng chọn <b>thành phố</b> và <b>chi nhánh</b> phù hợp với địa chỉ của bạn.</div>
// //             <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', marginTop: 10 }}>
// //               <select value={selectedCity} onChange={(e) => handleChooseCity(e.target.value)}>
// //                 <option value="">-- Chọn thành phố --</option>
// //                 {cityOptions.map(c => <option key={c} value={c}>{c}</option>)}
// //               </select>
// //               <select value={selectedBranch} onChange={(e) => handleChooseBranch(e.target.value)} disabled={!selectedCity}>
// //                 <option value="">-- Chọn chi nhánh --</option>
// //                 {branchesInCity.map(b => (
// //                   <option key={getBranchId(b)} value={getBranchId(b)}>{getBranchId(b)} - {getBranchAddress(b)}</option>
// //                 ))}
// //                 {selectedBranch && !branchesInCity.some(b => getBranchId(b) === selectedBranch) && (
// //                   <option value={selectedBranch}>{selectedBranchObj ? `${getBranchId(selectedBranchObj)} - ${getBranchAddress(selectedBranchObj)}` : (savedBranchLabel || selectedBranch)}</option>
// //                 )}
// //               </select>
// //               {(selectedCity || selectedBranch) && <button onClick={handleClearBranch} style={{ cursor: 'pointer' }}>Xóa chọn</button>}
// //             </div>
// //             {selectedBranchObj && (
...
// //             <>
// //               {/* GRID SẢN PHẨM (Dùng CSS cũ) */}
// //               <div className="product-grid-new" style={{ gridTemplateColumns: 'repeat(auto-fill, minmax(220px, 1fr))' }}> 
// //                 {pagedProducts.map((prod, idx) => {
// //                   // --- FIX ẢNH: Dùng đường dẫn thư mục ---
// //                   const imgSrc = `/images/products/${prod.MaMatHang}.jpg`;
...
// //                         <img src={imgSrc} alt={prod.TenMatHang} onError={(e) => { e.target.onerror = null; e.target.src = FALLBACK_IMG; }} />
// //                         {(prod.TinhTrang === 'Hết hàng' || outByBranch) && <span className="badge badge-out">Hết hàng</span>}
// //                       </div>
...
// //                           <p style={{ fontSize: '0.85rem', margin: '6px 0', color: Number(prod.SoLuongTon) > 0 ? '#2e7d32' : '#c62828' }}>Số lượng còn lại: {prod.SoLuongTon}</p>
// //                         )}
// //                         <div className="card-bottom">
// //                           <p className="card-price">{renderPrice(prod.DonGia)}</p>
// //                           <button className="btn-add-cart-mini" onClick={() => handleAddToCart(prod)} disabled={prod.TinhTrang === 'Hết hàng' || outByBranch || isEmployee} style={isEmployee ? {opacity: 0.3, cursor: 'not-allowed'} : {}} title="Thêm vào giỏ"> + </button>
// //                         </div>
// //                       </div>
// //                     </div>
// //                   );
// //                 })}
// //                 {pagedProducts.length === 0 && <p style={{ padding: 12 }}>Không có sản phẩm phù hợp.</p>}
// //               </div>

// //               {/* PHÂN TRANG */}
// //               <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', gap: 10, margin: '22px 0' }}>
// //                 <button onClick={() => goToPage(safePage - 1)} disabled={safePage === 1} style={{ border: 'none', background: 'transparent', fontSize: 20, cursor: 'pointer' }}>‹</button>
...
// //                       <button onClick={() => goToPage(p)} style={{ width: 38, height: 38, borderRadius: 6, border: '1px solid #eee', cursor: 'pointer', background: p === safePage ? '#e74c3c' : '#fff', color: p === safePage ? '#fff' : '#333', fontWeight: p === safePage ? 700 : 500 }}>{p}</button>
// //                     </React.Fragment>
// //                   );
// //                 })}
// //                 <button onClick={() => goToPage(safePage + 1)} disabled={safePage === totalPages} style={{ border: 'none', background: 'transparent', fontSize: 20, cursor: 'pointer' }}>›</button>
// //               </div>
// //             </>
// //           )}
// //         </div>
...
//   const [page, setPage] = useState(1);
//   const PAGE_SIZE = 12; // Tăng lên 10 vì đã bỏ sidebar, rộng chỗ hơn
...
//   const renderStars = (prod) => {
//       // Lấy điểm từ DB (Nếu ko có thì mặc định 0).
//       const rating = prod.DiemTrungBinh || 0; 
//       const count = prod.SoLuongDanhGia || 0; 
...
//       {/* --- CONTAINER CHÍNH --- */}
//       {/* display: block để bỏ layout flex cũ, giúp content tràn 100% */}
//       <div className="shop-container" style={{display: 'block'}}> 
...
//                 {pagedProducts.map((prod, idx) => {
//                   // --- 🔥 SỬA ĐÚNG CHỖ NÀY: Dùng ảnh trong thư mục public/images/products ---
//                   const imgSrc = `/images/products/${prod.MaMatHang}.jpg`;
...
const Products = () => {
  ...
  const [selectedCategory, setSelectedCategory] = useState(''); // loaiMH
  const [page, setPage] = useState(1);
  const PAGE_SIZE = 12; // Tăng nhẹ số lượng hiển thị vì đã bỏ cột lọc
  ...
  // --- HÀM XỬ LÝ THÊM GIỎ HÀNG (GIỮ NGUYÊN) ---
  const handleAddToCart = async (product) => {
    // --- 1. KIỂM TRA ĐĂNG NHẬP NGAY TỪ ĐẦU ---
    const token = localStorage.getItem('token');
    if (!token) {
  ...
  return (
    <div className="products-page-wrapper">
      ...
      {/* --- CONTAINER CHÍNH (Đã sửa display: block để bỏ layout flex cũ) --- */}
      <div className="shop-container" style={{display: 'block'}}> 
        
        {/* ĐÃ XÓA SIDEBAR (CỘT LỌC) Ở ĐÂY */}

        {/* CONTENT (Bây giờ nó sẽ full màn hình) */}
        <div className="shop-content" style={{width: '100%'}}>
          <div className="shop-header">
            <h2>SẢN PHẨM MỚI NHẤT</h2>
            <div className="search-bar-mini">
              <input
          ...
          {loading ? (
            ...
            <>
              {/* Grid sản phẩm tự động chia cột lại cho đều */}
              <div className="product-grid-new" style={{ gridTemplateColumns: 'repeat(auto-fill, minmax(220px, 1fr))' }}>
                {pagedProducts.map((prod, idx) => {
                  // --- GIỮ NGUYÊN LOGIC ẢNH CỦA BẠN (Đã sửa trước đó) ---
                  const imgSrc = `/images/products/${prod.MaMatHang}.jpg`;
