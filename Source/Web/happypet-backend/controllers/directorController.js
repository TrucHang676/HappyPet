...
exports.getProductRevenue = async (req, res) => {
    try {
        ...
        const result = await pool.request()
            ...
            .input('MaCN', sql.NChar(10), null) // NULL = tất cả chi nhánh
...
// 6. Cảnh báo hết hàng (toàn hệ thống hoặc theo chi nhánh)
exports.getLowStockProducts = async (req, res) => {
    try {
        ...
        const pool = await sql.connect();
        const result = await pool.request()
            .input('MaCN', sql.NChar(10), maCN || null) // NULL = tất cả chi nhánh
...
exports.updateMemberRanks = async (req, res) => {
    try {
        const { nam } = req.body;
...
// 10. Thống kê thú cưng theo loại, giống (toàn hệ thống)
exports.getPetsByType = async (req, res) => {
    try {
        const pool = await sql.connect();
        const result = await pool.request()
            .input('MaCN', sql.NChar(10), null) // NULL = toàn hệ thống
            .execute('sp_ThongKePetTheoLoai');
